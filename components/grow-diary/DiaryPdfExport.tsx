...
import { DiaryEvent, formatDiaryDate, groupEventsByDate, getEventTypeLabel } from '@/lib/diary-utils';
...
const DiaryPdfExport: React.FC<DiaryPdfExportProps> = ({ grow, events }) => {
  ...
  const generateTextExport = (): string => {
    ...
    lines.push('='.repeat(60));
    lines.push('');
    lines.push(`Started: ${formatDiaryDate(grow.startDate)}`);
    ...
    sortedDates.forEach(dateKey => {
      ...
      lines.push(`📅 ${formatDiaryDate(dateKey, {
        weekday: 'long', 
        year: 'numeric', 
        month: 'long', 
        day: 'numeric' 
  ...
  const generatePrintContent = (): string => {
    ...
    const eventRows = sortedDates.map(dateKey => {
      ...
      return `
        ...
        ${dayEvents.map(event => `
          <tr>
            <td style="padding: 8px; border-bottom: 1px solid #eee;">${escapeHtml(event.icon)}</td>
            <td style="padding: 8px; border-bottom: 1px solid #eee;">
              <strong>${escapeHtml(event.title)}</strong>
              ${event.plantName ? `<br><small style="color: #666;">🌿 ${escapeHtml(event.plantName)}</small>` : ''}
            </td>
            <td style="padding: 8px; border-bottom: 1px solid #eee;">${escapeHtml(event.description)}</td>
            <td style="padding: 8px; border-bottom: 1px solid #eee; color: #666; font-size: 12px;">
              ${escapeHtml(getEventTypeLabel(event.type))}
            </td>
          </tr>
        `).join('')}
      `;
    ...
    return `
      ...
      <html>
      <head>
        <title>Grow Diary - ${escapeHtml(grow.name)}</title>
        <style>
          body {
      ...
      <body>
        <h1>🌿 ${escapeHtml(grow.name)}</h1>
        <div class="meta">
          <p><strong>Started:</strong> ${escapeHtml(formatDiaryDate(grow.startDate))}</p>
          <p><strong>Current Phase:</strong> ${escapeHtml(grow.currentPhase)}</p>
        ...
        <div class="footer">
          <p>Generated by GrowPanion on ${escapeHtml(new Date().toLocaleString())}</p>
        </div>
      </body>
      </html>
    `;
